...
import { PiSpeakerHigh, PiStar, PiTrash } from "react-icons/pi";
...
export const SentenceList = ({ picker, toggleModal }: SentenceListType) => {
  ...
  return (
    <div className="flex flex-col gap-4 mt-10">
      {sentences.map(s => (
        <div onClick={() => openModal(s)} className="px-4 py-3 flex justify-between items-center gap-5 bg-slate-600 rounded-xl hover:shadow-md cursor-default smoother-2" key={s.createdAt}>
          <p className="w-96 overflow-hidden">{ s.sentence }</p>
          <div className="flex items-center gap-3">
            <button onClick={(e) => speakText(e, s.sentence)}><PiSpeakerHigh size={24} className="hover:text-sky-500 hover:drop-shadow-lg smoother-2" /></button>
